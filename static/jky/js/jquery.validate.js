 ...
 *
 * $Id: jquery.validate.js 8020 2009-06-02 10:46:19Z chengweidong $
 *
	...
	$.extend($.validator, {
		...
		prototype: {
			...
			elements: function() {
				var validator = this,
				rulesCache = {};
				var $a=$([]).add(this.currentForm.elements);
				if(IE==1){
					$a=$a.find(":input");
				}
				else{
				...
				return $a.not(":submit, :reset, :image, [disabled]").not(this.settings.ignore).filter(function() { ! this.name && validator.settings.debug && window.console && console.error("%o has no name assigned", this);
		...
		methods: {
			...
			remote: function(value, element, param) {
				...
				if (previous.old !== value) {
					...
					if(param.url.indexOf('http://')<0 && typeof CURURL!='undefined'){
						param.url=CURURL+param.url;
					}
			...
			checkTel: function(value, element, param) {
				...
				return /^[0-9\+(\s]{3,}[0-9\-)\s]{2,}[0-9]$/.test(value) ? true: false;
			},
			byteRange: function(value, element, param) {
				var length = value.length;
				var mb_length = param[2] == 'utf-8' ? 2 : 2;
...
function regTenpay(str) {
	if (regMobile(str) || regEmail(str) || !isNaN(str)) {
		return true;
	} else {
		return false;
	}
...
function dd_multiple(str, param) {
	param=String(param);
	a=param.split('.');
	if(typeof a[1]!='undefined'){
		param=parseFloat(param)*Math.pow(10,a[1].length);
		str=parseFloat(str)*Math.pow(10,a[1].length);
	}
...
String.prototype.subStr = function(len, s) {
	var str = '';
	var sp = s || '';
	var len2 = 0;
...
//var str1 = '按时打算打算';
//document.write('subStr(10) = ' + str1.subStr(10) + '');
jQuery.validator.addMethod("alipay",
function(value, element) {
	return this.optional(element) || regAlipay(value);
},
"支付宝格式错误");

jQuery.validator.addMethod("tenpay",
function(value, element) {
	return this.optional(element) || regTenpay(value);
},
"财付通格式错误");

jQuery.validator.addMethod("mobile",
function(value, element) {
	return this.optional(element) || regMobile(value);
},
"手机号码格式错误");

jQuery.validator.addMethod("dd_multiple",
function(value, element, param) {
	return this.optional(element) || dd_multiple(value, param);
},
"提现金额不是{0}的整数倍");
