...
;(function($){
    //联动菜单
    $.fn.mama_select = function(options) {
        var settings = {
            field: 'J_mama_id',
        ...
        self.nextAll('.J_mama_select').remove();
        $('<option value="">--'+settings.top_option+'--</option>').appendTo(self);
        $.getJSON(uri, {cid:pid}, function(result){
            if(result.status == '1'){
                for(var i=0; i< result.data.length; i++){
                $('<option value="'+result.data[i].cid+'">'+result.data[i].name+'</option>').appendTo(self);
                }
            }
        ...
        var j = 1;
        $('.J_mama_select').die('change').live('change', function(){
            var _this = $(this),
            _pid = _this.val();
            _this.nextAll('.J_mama_select').remove();
            if(_pid != ''){
                $.getJSON(uri, {cid:_pid}, function(result){
                    if(result.status == '1'){
                        var _childs = $('<select class="J_mama_select mr10" data-pid="'+_pid+'"><option value="">--'+settings.top_option+'--</option></select>')
                        for(var i=0; i<result.data.length; i++){
