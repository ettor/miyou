        ...
        public static T PopulateFromIDataReader<T>(DataRow dr)
        {
            ...
            for (int i = 0; i < dr.Table.Columns.Count; i++)
            {
                ...
                if (p != null)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(dr.Table.Rows[0][dr.Table.Columns[i].ToString()].ToString()))
                            p.SetValue(dynObj, Convert.ToDouble(dr.Table.Rows[0][dr.Table.Columns[i].ToString()].ToString()), null);
                    }
                    else if (p.PropertyType.Name == "Decimal")
                    {
                        if (!string.IsNullOrEmpty(dr.Table.Rows[0][dr.Table.Columns[i].ToString()].ToString()))
                            p.SetValue(dynObj, Convert.ToDecimal(dr.Table.Rows[0][dr.Table.Columns[i].ToString()].ToString()), null);
                    }
                }
            }
