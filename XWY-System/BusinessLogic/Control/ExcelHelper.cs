...
namespace BusinessLogic.Control
{
    public class ExcelHelper
    {
                ...
                if (rowIndex == 65535 || rowIndex == 0)
                {
...

                    {
                        ...
                        foreach (DataColumn column in dtSource.Columns)
                        {
                            ...
                            {
                                sheet.SetColumnWidth(column.Ordinal, 255);
                            }
        ...
        public static void AddRowExcel(string outputFile, string sheetname, string updateData, int coluid, int rowid)
        {
            FileStream readfile = new FileStream(outputFile, FileMode.Open, FileAccess.Read);
            ...
            try
            {
                sheet1.ShiftRows(rowid, sheet1.LastRowNum, 1);
                HSSFRow dataRow = sheet1.CreateRow(rowid) as HSSFRow;
                HSSFCell newCell = dataRow.CreateCell(coluid) as HSSFCell;
                HSSFCell newCell1 = dataRow.CreateCell(coluid + 1) as HSSFCell;
                if (!string.IsNullOrEmpty(updateData.Trim()))
                {
                    string[] vTemp = updateData.Split('：');

                    newCell.SetCellValue(vTemp[0]);
                    newCell1.SetCellValue(vTemp[1]);
        ...
        /// <param name="dtSource">源DataTable</param>
        /// <param name="strHeaderText">表头文本</param>
        static MemoryStream ExportDTOrder(DataTable dtSource, string strHeaderText)
        {
            HSSFWorkbook workbook = new HSSFWorkbook();
            HSSFSheet sheet = workbook.CreateSheet() as HSSFSheet;
                ...
                if (rowIndex == 65535 || rowIndex == 0)
                {
...

                    {
                        ...
                        foreach (DataColumn column in dtSource.Columns)
                        {
                            ...
                            try
                            {
                                sheet.SetColumnWidth(column.Ordinal, (arrColWidth[column.Ordinal] + 1) * 500);
                                //sheet.SetColumnWidth(column.Ordinal, 7550);
                            }
                            catch
                            {
                                sheet.SetColumnWidth(column.Ordinal, 255);
        ...
        /// <param name="strHeaderText">表头文本</param>
        /// <param name="strFileName">保存位置</param>
        public static void ExportDTtoExcelOrder(DataTable dtSource, string strHeaderText, string strFileName)
        {
            using (MemoryStream ms = ExportDTOrder(dtSource, strHeaderText))
            {
                using (FileStream fs = new FileStream(strFileName, FileMode.Create, FileAccess.Write))
                {
                    byte[] data = ms.ToArray();
