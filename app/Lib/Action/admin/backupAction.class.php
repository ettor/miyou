...
class backupAction extends BackendAction
{
    ...
    public function index()
    {
        if (IS_POST || isset($_GET['dosubmit']))
        {
            ...
            } else
            {
                ...
                if (is_dir(FTX_DATA_PATH . $this->backup_path . $this->backup_name))
                {
                    $this->error(L('backup_name') . L('exists'));
                }
                mkdir(FTX_DATA_PATH . $this->backup_path . $this->backup_name);

                if (!is_file(FTX_DATA_PATH . $this->backup_path . $this->backup_name .
                    '/tbl_queue.log'))
                {
                    //写入队列
    ...
    {
        $sql_file = FTX_DATA_PATH . $this->backup_path . $this->backup_name . '/' . $sql_file_name;
    ...
    private function _get_vols($backup_name)
    {
        $vols = array(); //所有的卷
        $bytes = 0;
        $vol_path = FTX_DATA_PATH . $this->backup_path . $backup_name . '/';
        if (is_dir($vol_path))
        {
    ...
    {
        $backups = array(); //所有的备份
        if (is_dir(FTX_DATA_PATH . $this->backup_path))
        {
            if ($handle = opendir(FTX_DATA_PATH . $this->backup_path))
            {
                while (($file = readdir($handle)) !== false)
                {
                    if ($file{0} != '.' && filetype(FTX_DATA_PATH . $this->backup_path . $file) == 'dir')
                    {
                        $backup['name'] = $file;
                        $backup['date'] = filemtime(FTX_DATA_PATH . $this->backup_path . $file) - date('Z');
						$backup['date'] = $backup['date']+28800;
                        $backup['date_str'] = date('Y-m-d H:i:s', $backup['date']);
                        $backup['vols'] = $this->_get_vols($file);
                        $end_vol = end($backup['vols']);
                        $backup['total_size'] =round($this->_get_dir_size(FTX_DATA_PATH . $this->backup_path . $file)/1024,2);
                        $backups[] = $backup;
                    }
                }
            }
    ...
    {
        $backup_path = FTX_DATA_PATH . 'backup/';
		$today	='ftxia_';
    ...
    {
        return file_put_contents(FTX_DATA_PATH . $this->backup_path . $this->backup_name .
    ...
    {
        $tbl_queue_file = FTX_DATA_PATH . $this->backup_path . $this->backup_name .
            '/tbl_queue.log';
    ...
    {
        $log_file = FTX_DATA_PATH . $this->backup_path . $this->backup_name . '/vol.log';
