...
class Page {
    ...
    public function show() {
        ...
        if ($downRow <= $this->totalPages){
            $downPage   =   "<a href='".str_replace('__PAGE__',$downRow,$url)."'>下一布</a>";
        }else{
            $downPage   =   '';
        }
        ...
        if ($this->totalPages > $this->rollPage) {
            if($this->nowPage - $middle < 1){
                ...
                $theFirst   =   "<a href='".str_replace('__PAGE__',1,$url)."' >第一页</a>";
            }
            if($this->nowPage + $middle > $this->totalPages){
                $theEnd     =   '';
            }else{
                $theEndRow  =   $this->totalPages;
                $theEnd     =   "<a href='".str_replace('__PAGE__',$theEndRow,$url)."' >最一页</a>";
            }
        }
	...
	public function show_1() {
		...
		$downRow = $this->nowPage+1;
		if ($upRow>0){
			$upPage="<a href='".$url."&".$p."=$upRow' class='prev'>上一页</a>";
		}else{
			$upPage="<span class='unprev'>上一页/span>";
		}

		if ($downRow <= $this->totalPages){
			$downPage="<a href='".$url."&".$p."=$downRow' class='next'>下一页</a>";
		}else{
			$downPage="<span class='unnext'>下一页</span>";
		}
		// << < > >>
		if($nowCoolPage == 1){
			$theFirst = "";
    ...
    public function zshow() {
        ...
        $downRow        =   $this->nowPage+1;
        if ($upRow>0){
            $upPage     =   "<span class='prev'><a href='".str_replace('__PAGE__',$upRow,$url)."'  rel='prev' style='width:100px;'>上一页</a></span>";
        }else{
            $upPage     =   '';
        }

        if ($downRow <= $this->totalPages){
            $downPage   =   "<span class='next'><a href='".str_replace('__PAGE__',$downRow,$url)."' rel='next' style='width:100px;'>下一页</a></span>";
        }else{
            $downPage   =   '';
        }
    ...
    public function mshow() {
        ...
        $downRow        =   $this->nowPage+1;
        if ($upRow>0){
            $upPage     =   "<a href='".str_replace('__PAGE__',$upRow,$url)."' class='prev'>上一页</a>";
        }else{
            $upPage     =   "";
        }

        if ($downRow <= $this->totalPages){
            $downPage   =   "<a href='".str_replace('__PAGE__',$downRow,$url)."' class='next'>下一页</a>";
        }else{
            $downPage   =   "";
        }
