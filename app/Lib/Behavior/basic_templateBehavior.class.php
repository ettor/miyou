    ...
    private function parseTemplateFile($templateFile) {
        ...
        if(''==$templateFile) {
            ...
            if(!file_exists_case($templateFile) && C('DEFAULT_THEME') && C('BASIC_THEME')) {
                ...
                $ftx_default_theme = C('BASIC_THEME');
                $group = defined('GROUP_NAME') ? GROUP_NAME . '/' : '';
                $ftx_theme_path = TMPL_PATH . $group . $ftx_default_theme . '/';
                $templateFile = $ftx_theme_path.MODULE_NAME.(defined('GROUP_NAME')?C('TMPL_FILE_DEPR'):'/').ACTION_NAME.C('TMPL_TEMPLATE_SUFFIX');
            }
